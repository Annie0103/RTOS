...
printout buffer[BUFFER_SIZE];
INT32U  buffer_index=0;
INT32U  ii=0;

void print_buffer(void) {
    //printf("Time\tInformation\t\t\tfrom\tto\n");
    printf("----------------------------------------------------------\n");
    for (ii = 0; ii < buffer_index; ii++) {
        printf("%ld\t%s\t\t\t%d\t%d\n", buffer[ii].tt, buffer[ii].info, buffer[ii].from, buffer[ii].to);
    }
}
/*void print_buffer(void) {
    INT8U *p_data;
    ...
    OS_ENTER_CRITICAL();    // 進入關中斷保護區
    p_data = &buffer[0];
    for (i = 0; i < BUFFER_SIZE; i++) {//BUFFER_SIZE
        if (*p_data != 0) {    // 判斷buffer是否為空值
            printf("%s\n", p_data);    // 印出buffer內容
        }
        p_data += PRINT_BUF_SIZE;
    }
    OS_EXIT_CRITICAL();    // 離開關中斷保護區
}*/
...
void  OSIntExit (void)
{
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            //printf("\n%5d Preempt %5d",OSTime, OSTCBCur->OSTCBPrio);
            ...
            //if(cc<100){
                //OSTCBCur->printTag = 1;
                //buffer[cc].c = 1;
                //buffer[cc].from = OSPrioCur;
                //buffer[cc].to = OSPrioHighRdy;
                //buffer[cc].tt = OSTime;
                //(OSTCBCur->cc)++;    
            //}

            //sprintf(s,"%5d       preemption           %5d             %5d          ", OSTime,OSPrioCur,OSPrioHighRdy);
            //PC_DispStr(0, ((OSTCBCur->cc)%21 + 1), s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);        
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                //printf("\n%5d Preempt %5d %5d",OSTime, OSPrioCur, OSPrioHighRdy); 
                OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy];
                OSCtxSwCtr++;                              /* Keep track of the number of ctx switches */

                if (buffer_index < BUFFER_SIZE) {
                    buffer[buffer_index].tt = OSTimeGet();
                    buffer[buffer_index].info = "preemption\0";
                    buffer[buffer_index].from = OSTCBCur->OSTCBPrio;
                    buffer[buffer_index].to = OSPrioHighRdy;
                    buffer_index++;
                }
                else{
                    buffer_index = 0;  
...
void  OS_Sched (void)
{
    ...
    if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Sched. only if all ISRs done & not locked    */
        ...
        //printf("\n%5d complete %5d %5d",OSTime, OSPrioCur, OSPrioHighRdy); //OSTCBCur->OSTCBPrio  
        //if(cc<100){   
            //OSTCBCur->printTag = 0;
            //buffer[cc].c = 0;
            //buffer[cc].from = OSPrioCur;
        ...
        //sprintf(s,"%5d       complete           %5d             %5d               ", OSTime,OSPrioCur,OSPrioHighRdy);
        //PC_DispStr(0, ((OSTCBCur->cc)%21 + 1), s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
        if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy     */
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
            OSCtxSwCtr++;                              /* Increment context switch counter             */
                ...
                if (buffer_index < BUFFER_SIZE) {
                    buffer[buffer_index].tt = OSTimeGet();
                    buffer[buffer_index].info = "complete\0";
                    buffer[buffer_index].from = OSTCBCur->OSTCBPrio;
                    buffer[buffer_index].to = OSPrioHighRdy;
                    buffer_index++;
                }
